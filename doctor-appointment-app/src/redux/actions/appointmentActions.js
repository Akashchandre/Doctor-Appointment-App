...
export const fetchAppointments = () => async (dispatch) => {
  try {
   ...
   if (!token) {
    ...
    const res = await axios.get("https://doctor-appointment-app-sz3z.onrender.com/api/appointments", {
      headers: {
...
export const bookAppointment = (appointmentData) => async (dispatch) => {
  try {
    ...
    const res = await axios.post("https://doctor-appointment-app-sz3z.onrender.com/api/appointments", appointmentData, {
      headers: {
        Authorization: `Bearer ${token}`, 
      },
    });
